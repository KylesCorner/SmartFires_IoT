...
5. **If total node count exceeds 4, also bump `kMaxPendingCommands` in `SmartFiresBaseApp.h`** (currently `4`) and reflash the base. The Jetson's "New Session" web flow hard-resets every configured node by queueing one `CMD_RESET` per node on the base — beyond 4 queued at once, the extras silently fail to enqueue (`QUEUE_FULL`) and never get reset. See `documentation/Pending_Plans/RESET_SYSTEM.md`'s "Known Limitation" section.
