...
// OledPageController::OledPageController(DroneContext& ctx, AppState& state, IClock& clock)
...
void OledPageController::renderEnv() {
  _ctx.oled.printLine(0, _state.sensingEnabled ? "Environment" : "Environment [PAUSED]");

  if (_ctx.wind.hasReading()) {
    _ctx.oled.printfLine(1, "Wind Speed:%.3f", _ctx.wind.windMps());
  } else {
    _ctx.oled.printLine(1, _state.sensingEnabled ? "Wind: Warming up" : "Wind: paused");
  }

  if (_ctx.sht31.hasReading()) {
    _ctx.oled.printfLine(2, "T:%.1fF %.1fC H:%.1f%%",
                         _ctx.sht31.temperatureF(),
                         _ctx.sht31.temperatureC(),
                         _ctx.sht31.humidityPct());
  } else {
    _ctx.oled.printLine(2, _state.sensingEnabled ? "T/H:no reading" : "T/H: paused");
  }

  if (_ctx.flame.hasReading()) {
    _ctx.oled.printfLine(3, "Flame:%d", _ctx.flame.analogRaw());
  } else {
    _ctx.oled.printLine(3, _state.sensingEnabled ? "No flame reading" : "Flame: paused");
  }
}
...
void OledPageController::renderGps() {
  ...
  if (!_ctx.gps.hasReading()) {
    ...
    _ctx.oled.printfLine(2, "Age:%lu ms", (unsigned long)_ctx.gps.sentenceAgeMs());
    _ctx.oled.printLine(3, "Go outside");
  } else {
    _ctx.oled.printfLine(1, "S:%u Alt:%.0fm",
                         _ctx.gps.satellites(),
...
void OledPageController::renderUart() {
  ...
  _ctx.oled.printfLine(2, "WaitAck:%s", _state.link.waitingForAck ? "YES" : "NO");
...
void OledPageController::renderLora() {
  ...
  const bool linkAlive =
      bootSeen &&
      ((_clock.millis() - _state.link.lastSendTimeMs) < 3000 ||
       _state.link.waitingForAck == false);

  _ctx.oled.printfLine(1, "B:%s L:%s",
                       bootSeen ? "Y" : "N",
                       linkAlive ? "OK" : "WAIT");

  _ctx.oled.printfLine(2, "S:%lu A:%lu",
                       (unsigned long)_state.link.lastSentSeq,
                       (unsigned long)_state.link.lastAckedSeq);
