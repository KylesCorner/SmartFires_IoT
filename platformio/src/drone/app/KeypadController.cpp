...
void KeypadController::update() {
  ...
  case KEYPAD_PREV_PAGE: {
    ...
    _state.ui.currentPage =
        static_cast<OledPage>((current == 0) ? (count - 1) : (current - 1));
  ...
  case KEYPAD_TOGGLE_CONT:
    _state.continousMode = !_state.continousMode;
    _state.ui.oledNeedsRefresh = true;
    break;
