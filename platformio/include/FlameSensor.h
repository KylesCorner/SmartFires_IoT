...
public:
  ...
  FlameSensor(uint8_t analogPin, uint8_t digitalPin,
      ...
      : _analogPin(analogPin), _digitalPin(digitalPin), _polarity(polarity),
        _name(sensorName) {}
  ...
  bool ready() const override { return (millis() >= _warmupUntilMs); }
  ...
  bool sample() override {
    ...
    if (now - _lastSampleMs < _minIntervalMs)
      return false;
...
private:
  ...
  bool interpretDigital(bool raw) const {
    // raw == true means HIGH
    if (_polarity == DigitalPolarity::ActiveLow)
      return raw;
    return !raw; // active-low
  }

  bool interpretAnalog(int raw) const {
    // Many modules: more IR -> lower AO voltage (often raw decreases near
    // flame), but this varies. We let you choose direction.
    if (_detectWhenBelow)
