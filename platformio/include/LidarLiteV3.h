...
public:
  ...
  explicit LidarLiteV3(uint8_t addr = kDefaultAddress,
                       const char *sensorName = "LIDAR-Lite v3",
                       uint32_t ioTimeoutMs = 50,
                       uint8_t maxFailuresBeforeUnhealthy = 3)
      : _addr(addr), _name(sensorName), _ioTimeoutMs(ioTimeoutMs),
  ...
  void setMeasurementBiasCorrection(bool enable) {
    _useBiasCorrection = enable;
  }
